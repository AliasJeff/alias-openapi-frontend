...
const Index: React.FC = () => {
    ...
    tab1: [
      {
        ...
        dataIndex: 'name',
        align: 'center',
        render: (_, record) => <Tooltip title="详细信息"><a onClick={() => {
          setIsModalOpen(true), setData(record)
        }}>{_}</a></Tooltip>
      },
      {
        title: '描述',
  ...
  return (
    <PageContainer>
      <ProTable<API.InterfaceInfo>
        ...
        request={async (params, sorter, filter) => {
          ...
          if (activeKey === "tab1") {
            ...
            const res = await getAvailableInterfaceInfoUsingGET({...params, ...sorter})
            if (res?.data) {
              setTableListDataSource(res?.data?.records)
              return {
                data: res?.data?.records,
      ...
      } destroyOnClose={true} title="详细信息" open={isModalOpen} onCancel={() => {
        setIsModalOpen(false)
      }}>
      ...
      <Modal title="确认订单" open={confirmOrderModal} onCancel={() => {
        setConfirmOrderModal(false), setInputNumberValue(1)
      }} footer={[
        <Button onClick={() => {
          setConfirmOrderModal(false), setInputNumberValue(1)
        }}>
        ...
        <Descriptions column={1} bordered>
          ...
          <Descriptions.Item label="计费规则">{confirmData?.price} 元/条</Descriptions.Item>
          <Descriptions.Item label="购买次数">
            <InputNumber min={1} value={inputNumberValue} defaultValue={1}
                         onChange={(value: any) => {
                           setInputNumberValue(value), setTotalNumMoney(value, confirmData?.price)
                         }}/>
          </Descriptions.Item>
