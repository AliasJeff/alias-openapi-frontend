...
const Order: React.FC = () => {
  ...
  const createAlipay = async () => {
    const res = await payUsingPOST({
      alipayTraceNo: orderData?.orderSn,
      subject: orderData?.name,
      totalAmount: orderData?.totalAmount,
      traceNo: orderData?.orderSn
    })
    ...
    <PageContainer style={{position: "relative", height: '100%', width: '100%'}}>
      ...
      >
        <Descriptions column={1}>
          ...
          <Descriptions.Item label="总价" labelStyle={{fontSize: 15, marginTop: 4}}
                             contentStyle={{fontSize: 20}}>{orderData?.totalAmount} 元</Descriptions.Item>
          ...
          <Descriptions.Item label="剩余支付时间" labelStyle={{marginTop: 8, fontSize: 15}}><Tag color="cyan"><Countdown
            loading={countDownLoading} value={deadline} onFinish={() => {
            history.push("/Order/order", null), message.warning("订单已失效，请重新提交订单")
          }} format="HH:mm:ss:SSS"/></Tag></Descriptions.Item>
          <Descriptions.Item label="支付方式" labelStyle={{fontSize: 15, marginTop: 8}}><img
            src="https://gw.alipayobjects.com/mdn/member_frontWeb/afts/img/A*oRlnSYAsgYQAAAAAAAAAAABkARQnAQ"/></Descriptions.Item>
        </Descriptions>
        ...
        <div style={{float: "right", display: "flex"}}>
          ...
          <Button type={"primary"} onClick={createAlipay}> <Tooltip
            title={"需要支付：" + orderData?.totalAmount + " 元"}>
            立即支付
          </Tooltip></Button>
        </div>
      </ProCard>
      <Modal title="取消订单" open={isModalOpen} onOk={async () => {
        await cancelOrderSnUsingPOST({
          interfaceId: orderData?.interfaceId,
          orderNum: orderData?.orderNum,
          orderSn: orderData?.orderSn
        }) , history.push("/getRequestCounts", null), message.success("订单取消成功")
      }} onCancel={() => {
